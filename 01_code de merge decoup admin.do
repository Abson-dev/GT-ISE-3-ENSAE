...
****************************************************************
					...
					if "`ref_code_excel'" == "`ref_code_folder'" {
						...
						cd "`output_dir'/`subfolder'"
								...
								foreach stata_file of local stata_files {
									if strpos("`stata_file'", "_welfare_individu") {
									
										// Importons la base "taux_pauvreté_commune"
										use "`output_dir'/`subfolder'/`stata_file'", clear
										//save `stat_poverty_data', replace
										
										** Recherchons  les variables clés et renommons les
										
										//////////////////////////////////
										//capture drop comm  depart
			...
			*************************************************************************************************************************************************************	
										tempfile stat_poverty_data

										save `stat_poverty_data', replace  // Sauvegarder la version harmonisée de ehcvm_individu
										save "`output_dir'/`subfolder'/stat_poverty_data_`ref_code_excel'2021", replace
									...
									*************************************************************************************************	
										...
										use `stat_poverty_data', clear
										merge m:1 commune_clean departement_clean  using `hdx_data', keep(3) 
										drop _merge
								   ****************************************************************************************************

										// 7. SAUVEGARDER LA BASE FINALE OBTENUE
										save "`output_dir'/`subfolder'/communal_statpoverty_`ref_code_excel'2021", replace
									}
								}
							}
						}
