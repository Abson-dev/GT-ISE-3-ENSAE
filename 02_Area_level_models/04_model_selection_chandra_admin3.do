...
//Removal of non-significant variables
	...
	quietly:fhsae dir_fgt0 ${postsign}, revar(dir_fgt0_var) method(chandra)
...
dis as error "Sim : `sim' final removal"
	...
	forval z= 0.2(-0.05)1e-5{
		local regreso 
		while ("`regreso'"!="it's done"){
			quietly:fhsae dir_fgt0 `hhvars', revar(dir_fgt0_var) method(chandra) precision(1e-10)
			mata: bb=st_matrix("e(b)")
			mata: se=sqrt(diagonal(st_matrix("e(V)")))
			local _myhhvars : colnames(e(b))
			mata: st_local("regreso", invtokens(mysel2(bb, se, `z')))	
...
//*********************************************************************************************//
	...
	fhsae dir_fgt0  $postsign, revar(dir_fgt0_var) method(chandra) fh(fh_fgt0) ///
	...
	graph export "$figs\Fig1_left_CHANDRA.png", as(png) replace
	...
	graph export "$figs\Fig1_right_CHANDRA.png", as(png) replace
	qnorm e_d, graphregion(color(white))

 reg dir_fgt0 $postsign, r
	
	estimates store model
	esttab model using "$data_output\model_CHANDRA.rtf"	, replace
...
save "$data_output\FH_sae_poverty_CHANDRA.dta", replace
