...
//*********************************************************************************************//
	...
	graph export "$figs\Fig1_left_REML.png", as(png) replace
	...
	graph export "$figs\Fig1_right_REML.png", as(png) replace
	qnorm e_d, graphregion(color(white))

 reg dir_fgt0 $postsign, r
	
	estimates store model
	esttab model using "$data_output\model_REML.rtf"	, replace
...
save "$data_output\FH_sae_poverty_REML.dta", replace
