...
merge 1:1 adm3_pcode using "$data_output\direct_survey_ehcvm_bfa_2021_admin3.dta"
/*
...
fhsae dir_fgt0 `hhvars', revar(dir_fgt0_var) method(ml)
...
//Removal of non-significant variables
	...
	quietly:fhsae dir_fgt0 ${postsign}, revar(dir_fgt0_var) method(ml)
	
	//Check VIF
	reg dir_fgt0 $postsign, r
	/*
	estimates store model
	esttab model using "$data_output\model.rtf"
	*/
	gen touse = e(sample)
	gen weight = 1
...
dis as error "Sim : `sim' final removal"
	...
	forval z= 0.2(-0.05)1e-5{
		local regreso 
		while ("`regreso'"!="it's done"){
			quietly:fhsae dir_fgt0 `hhvars', revar(dir_fgt0_var) method(ml) precision(1e-10)
			mata: bb=st_matrix("e(b)")
			mata: se=sqrt(diagonal(st_matrix("e(V)")))
			local _myhhvars : colnames(e(b))
			mata: st_local("regreso", invtokens(mysel2(bb, se, `z')))	
...
//*********************************************************************************************//
	...
	fhsae dir_fgt0  $postsign, revar(dir_fgt0_var) method(ml) fh(fh_fgt0) ///
	...
	graph export "$figs\Fig1_left_ML.png", as(png) replace
	...
	graph export "$figs\Fig1_right_ML.png", as(png) replace
	qnorm e_d, graphregion(color(white))

 reg dir_fgt0 $postsign, r
	
	estimates store model
	esttab model using "$data_output\model_ML.rtf"	, replace
...
save "$data_output\FH_sae_poverty_ML.dta", replace
