...
//*********************************************************************************************//
	...
	fhsae dir_fgt0  $postsign, revar(dir_fgt0_var) method(fh) fh(fh_fgt0) ///
	...
	graph export "$figs\Fig1_left_FH.png", as(png) replace
	...
	graph export "$figs\Fig1_right_FH.png", as(png) replace
	qnorm e_d, graphregion(color(white))

 reg dir_fgt0 $postsign, r
	
	estimates store model
	esttab model using "$data_output\model_FH.rtf"	, replace
...
save "$data_output\FH_sae_poverty_FH.dta", replace
